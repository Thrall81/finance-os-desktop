...
| ADR-145 | DatePicker Unity App UI (com.unity.dt.app-ui) confirmé fonctionnel sur un premier champ (date de début, Opérations récurrentes) ; généralisation aux autres champs date pas encore faite | ACCEPTED |
...
# 47. ADR-145 — DatePicker Unity App UI, confirmé fonctionnel sur un premier champ
...
**Décision** : installé (`com.unity.dt.app-ui` 2.2.2, stable — la version `3.0` documentée par l'utilisateur est encore en pré-version `3.0.0-pre.1`, écartée pour un projet qui vise la stabilité), câblé sur un seul champ (« Date de début », Opérations récurrentes, création uniquement) pour valider la faisabilité réelle avant de toucher les ~7 autres champs date de l'app. **Confirmé visuellement fonctionnel par l'utilisateur après quatre correctifs réels**, détaillés ci-dessous — accepté pour ce premier champ ; la généralisation aux autres champs date de l'application reste à faire dans une prochaine session.
...
**Restant hors périmètre de cette entrée** : la généralisation aux ~7 autres champs date de l'application (Transactions, filtres, historique de solde, Prévisions ×3) — chacun aura besoin du même câblage (`AppUiThemeStyleSheet` transmis au contrôleur concerné). App UI expose aussi un composant `DateField` tout fait (`Runtime/UI/Components/DateField.cs`, combine champ + calendrier) qui n'a pas été essayé — pourrait éviter une partie de `AppDatePickerField` fait main, à évaluer avant la généralisation plutôt qu'après.
