...
| ADR-145 | Essai du DatePicker Unity App UI (com.unity.dt.app-ui) sur un seul champ (date de début, Opérations récurrentes) avant généralisation | PROPOSED |
...
# 47. ADR-145 — Essai du DatePicker Unity App UI (PROPOSED, non généralisé)
...
**Décision, à ce stade — essayer sur un seul champ avant tout engagement plus large** : installé (`com.unity.dt.app-ui` 2.2.2, stable — la version `3.0` documentée par l'utilisateur est encore en pré-version `3.0.0-pre.1`, écartée pour un projet qui vise la stabilité), câblé sur un seul champ (« Date de début », Opérations récurrentes, création uniquement) pour valider la faisabilité réelle avant de toucher les ~7 autres champs date de l'app.

**Risque technique initial levé en lisant le vrai code source du paquet, pas la documentation** : la crainte de devoir envelopper toute l'application dans le `Panel` racine propre à App UI (son propre système de layers popup/notification/tooltip) s'est révélée infondée — `Popup.GetRootPopupLayer` (dans le paquet, `Runtime/UI/Popups/Popup.cs`) retombe explicitement sur la racine du visual tree existant quand aucun `Panel` App UI n'est trouvé (`Panel.FindPopupLayer(element) ?? element?.GetExclusiveRootElement()`). `Popover.Build(referenceView, contentView)` fonctionne donc directement dans le `UIDocument` déjà en place, sans restructuration.

**Ce qui a été construit** : `AppDatePickerField` (nouveau, `FinanceOS.UI`) — attache un clic sur un `Button` stylé pour ressembler à un champ (`.date-field-trigger`, nouvelle classe USS, pas de style de bouton) plutôt qu'un `VisualElement` brut, pour garantir que le clic se déclenche réellement (même prudence que partout ailleurs dans ce projet face à un événement non vérifiable sans panel réel). Au clic : construit un `DatePicker` App UI, lui applique les classes de contexte de thème (`appui--dark`/`appui--light` selon `AppSettings.Theme`, plus `appui--medium` pour l'échelle) — nécessaire car le Popover est ajouté à la racine du panel, pas sous `.shell-root`, donc n'hérite jamais du basculement `.theme-dark` propre à cette app — l'ouvre dans un `Popover` ancré au bouton, et referme le popover au choix d'une date. `RecurringOperations.uxml` référence directement les feuilles de style du paquet (`App UI.uss`, thèmes clair/sombre, échelle medium) plutôt que de passer par le slot unique `PanelSettings.themeStyleSheet` — qui contient déjà le thème runtime par défaut de Unity et ne peut porter qu'une seule feuille de thème à la fois — pour ne prendre aucun risque sur le thème du reste de l'application.

**Trois erreurs de compilation réelles, trouvées et corrigées, pas anticipées** : (1) `FinanceOS.UI.asmdef` n'avait aucune référence vers l'assembly du paquet (`Unity.AppUI`) — ajoutée. (2) App UI a son propre `Unity.AppUI.UI.Button`, strictement ambigu avec `UnityEngine.UIElements.Button` dès que les deux espaces de noms sont importés ensemble (`error CS0104` réel) — résolu par un alias explicite (`using Button = UnityEngine.UIElements.Button;`) pour garder le Button UI Toolkit standard, le seul utilisé ailleurs dans ce projet. (3) Le type `Date` vit dans `Unity.AppUI.Core`, pas `Unity.AppUI.UI` — using manquant, ajouté.
...
**Ce qui reste vérifiable en batchmode, et ce qui ne l'est pas — même limitation ADR-112 que tout le reste de ce projet, mais plus aiguë ici** : la compilation, l'assemblage de l'API réelle du paquet (confirmée en lisant son code source, pas devinée) et l'état structurel après `OpenCreateForm` (le texte du bouton passe bien du placeholder à la date du jour) sont vérifiés et verts. **Rien de tout le reste ne l'est** : le clic lui-même (déclenche `Popover.Show()`, jamais simulable ici), le rendu réel du calendrier (couleurs, lisibilité, alignement des classes de thème appliquées manuellement), le positionnement du popover par rapport au champ, et la cohérence visuelle avec le reste de l'app (App UI a son propre système de tokens `--appui-*`, entièrement séparé des `--color-*` de ce projet). **Statut PROPOSED, pas ACCEPTED** : à confirmer par une vraie capture d'écran de l'utilisateur avant toute généralisation aux ~7 autres champs date de l'application (Transactions, filtres, historique de solde, Prévisions ×3).

**Documents concernés** : aucun — décision non actée tant que non validée visuellement.
