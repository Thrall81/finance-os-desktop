...
**Décision** : retirer `skipifsilent` de la ligne `[Run]` — `postinstall` seul suffit, et s'exécute automatiquement aussi bien en installation silencieuse qu'interactive (où il reste, comme avant, une case à cocher pré-cochée sur la dernière page de l'assistant). C'est la ligne `[Run]`, pas `CloseApplications`/`RestartApplications`, qui relance réellement l'application après une auto-mise-à-jour désormais — ces deux directives restent utiles pour un scénario différent et bien réel (l'installeur relancé manuellement pendant que l'application tourne encore), documenté explicitement dans un commentaire réécrit plutôt que laissé à réinterpréter plus tard.

**Pourquoi ce correctif suffit sans toucher au C# déjà déployé** : le correctif vit entièrement dans le script Inno Setup, compilé dans chaque nouvel installeur téléchargé, pas dans le code C# de l'application actuellement installée chez l'utilisateur. La toute prochaine auto-mise-à-jour (même déclenchée par une version antérieure encore affectée par le bug) télécharge et exécute l'installeur de la version corrigée, qui contient déjà la ligne `[Run]` réparée — aucune version intermédiaire ni changement de comportement côté `AppBootstrap.cs` n'est nécessaire pour que la relance fonctionne dès la prochaine mise à jour.

**Vérification empirique, pas seulement du raisonnement** : le même test PowerShell d'installation/désinstallation silencieuse déjà utilisé à chaque version ce mois-ci a été étendu pour vérifier concrètement qu'un processus `FinanceOS.exe` démarre bien après une installation `/VERYSILENT` — la seule façon de confirmer ce correctif sans se fier uniquement à la sémantique documentée d'Inno Setup.
