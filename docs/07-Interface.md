...
- une infobulle apparaît au survol d'un point/d'une barre (valeur exacte, date, détail) — **pas encore construite** sur aucun des quatre graphiques : demanderait une détection de position de pointeur sur un rendu vectoriel, différée tant que le tracé de base n'est pas confirmé visuellement (cf. ADR-119) ;
