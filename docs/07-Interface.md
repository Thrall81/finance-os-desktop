...
**Navigation** : une barre latérale (`Shell.uxml` + `ShellController`) donne accès au Tableau de bord, à Comptes, à Transactions, à Opérations récurrentes et à Prévisions ; chaque écran est un `VisualTreeAsset` instancié dans la zone de contenu du shell plutôt qu'une scène séparée (cf. `09-Decisions_techniques.md`, ADR-113). Les autres entrées listées en §3 (Budget, Paramètres) n'ont pas encore d'entrée dans la barre latérale.
